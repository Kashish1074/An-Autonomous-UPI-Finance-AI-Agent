...
PaisaPilot uses Claude's tool-use (function calling) in a real loop:

1. The user asks a question in plain English/Hindi-English mix
   (e.g. "How much did I spend on food last month?")
2. Claude decides which tool(s) to call: `get_total_spend`,
...
3. The app executes that tool against the actual pandas dataframe of
   transactions
4. The result is returned to Claude, which grounds its final answer in the
...
4. Deploy — you'll get a public `*.streamlit.app` URL, no login required

## Tech stack

- **Agent brain:** Claude (Anthropic API, tool use / function calling)
- **App:** Streamlit
- **Data:** pandas
- **Charts:** Plotly
